...
def plot_staggered_points():
    ...
    lon_t = grid_data['glamt'][0, :, :]
    lat_u = grid_data['gphiu'][0, :, :]
    lon_u = grid_data['glamu'][0, :, :]
    lat_v = grid_data['gphiv'][0, :, :]
    lon_v = grid_data['glamv'][0, :, :]
    lat_f = grid_data['gphif'][0, :, :]
    lon_f = grid_data['glamf'][0, :, :]
    ...
    plt.suptitle('Arakawa C-Grid Staggered Points', y=1.02)
    plt.tight_layout()
    plt.savefig('staggered_points.png', dpi=300)
    plt.close()
...
def generate_all_plots():
    ...
    for res in ['2deg', '1deg', '0.5deg']:
        ...
        if os.path.exists(grid_file):
            ...
            plot_grid_structure(grid_file, f'{res} ORCA Grid', f'plots/{res}_grid')
            plot_scale_factors(grid_file, f'{res} ORCA Grid', f'plots/{res}_scale_factors')
        else:
