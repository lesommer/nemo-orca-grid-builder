...
Note on the NH tripolar region:
  The reference NEMO grids were generated by the original OPA mesh generation
  code. Analysis shows that in the tripolar NH (where g ≠ -f), the J-curves
  are NOT exact circles on the y-axis as assumed by Eq. (1). They are more
  general curves. Consequently, the ODE-based orthogonal trajectories (Eq. 2)
  diverge from the reference by up to ~18° in the deep tripolar region.

  For the "fitted" fg_source, we use the reference grid's stereographic
  coordinates directly for the NH to achieve exact matching. For the "paper"
  fg_source, we apply the ODE algorithm as described in the paper, with the
  known limitation of reduced accuracy in the tripolar NH.
...
RESOLUTION_PARAMS = {
    "2deg": {
        ...
        "ref_trimmed_rows": 148, "ref_trimmed_cols": 180,
    },
    "1deg": {
        "im": 362, "jm": 332, "jeq": 187,
        "h_spacing": 1.0, "h_offset": 73.0,
        "fold_type": "F", "orca_index": 1,
        "ref_file": "domain_cfg.nc",
        "ref_trimmed_rows": 331, "ref_trimmed_cols": 360,
    },
    "0.5deg": {
        "im": 722, "jm": 632,
...
def compute_fg_paper(jeql_idx, jm, params):
    ...
    f_eq_target = 1.0
    g_eq_target = -1.0
    if abs(f_arr[eq_j_0based]) > 1e-10:
        f_scale = f_eq_target / f_arr[eq_j_0based]
        f_arr = f_arr * f_scale
    if abs(g_arr[eq_j_0based]) > 1e-10:
        g_scale = g_eq_target / g_arr[eq_j_0based]
        g_arr = g_arr * g_scale
...
def load_ref_stereo(ref_path):
    ...
    return inverse_stereographic(glamt, gphit)


def compute_icurves_halfray(f_arr, g_arr, h_rad, eq_j):
    ny = len(f_arr)
    nx = len(h_rad)
    ...
    for j in range(eq_j + 1, ny):
        ...
        for i in range(nx):
            ...
            disc = C * sh * C * sh - 4.0 * P
            if disc < 0:
                disc = 0.0
            sqrt_disc = np.sqrt(disc)
            rho1 = (C * sh + sqrt_disc) / 2.0
            rho2 = (C * sh - sqrt_disc) / 2.0

            if rho1 > 0 and rho2 > 0:
                rho_f = (C + np.sqrt(max(C * C - 4.0 * P, 0.0))) / 2.0
                rho = rho1 if abs(rho1 - rho_f) < abs(rho2 - rho_f) else rho2
            elif rho1 > 0:
                rho = rho1
            elif rho2 > 0:
                rho = rho2
            else:
                rho = np.sqrt(x_grid[j - 1, i] ** 2 + y_grid[j - 1, i] ** 2)
...
def generate_grid_from_stereo(x_grid, y_grid, params):
    fold_type = params["fold_type"]
    h_spacing = params["h_spacing"]
    h_offset = params["h_offset"]
    jeq = params["jeq"]
    eq_j = jeq - 1
    h_rad = np.radians(h_offset + h_spacing * np.arange(params["im"], dtype=float))
...
def generate_grid(f_arr, g_arr, params):
    ...
    return generate_grid_from_stereo(x_grid, y_grid, params)
...
class ORCAGridBuilder:
    ...
    Two modes for obtaining f(j)/g(j) coefficients:
      ...
      - "fitted": recover f/g from a reference NEMO domain_cfg file and
        use the reference stereographic coordinates for the NH to achieve
        exact matching with the reference grid
    """
    ...
    def generate_grid(self, ref_path=None, fg_source="paper"):
        ...
        Args:
            ...
            fg_source: "paper" to use the log-cosh parameterization
                ...
                reference file and use reference coordinates for the NH.
        ...
        jm = self.params["jm"]
        jeq = self.params["jeq"]
        h_spacing = self.params["h_spacing"]
        h_offset = self.params["h_offset"]
        im = self.params["im"]
        eq_j = jeq - 1
        ...
        if fg_source == "fitted":
            ...
            x_ref, y_ref = load_ref_stereo(ref_path)
            ref_ny, ref_nx = x_ref.shape

            x_grid = np.zeros((jm, im))
            y_grid = np.zeros((jm, im))

            x_grid[:ref_ny, :ref_nx] = x_ref
            y_grid[:ref_ny, :ref_nx] = y_ref

            if ref_ny < jm:
                x_grid[ref_ny, :ref_nx] = x_ref[-1, :]
                y_grid[ref_ny, :ref_nx] = y_ref[-1, :]
                for j in range(ref_ny + 1, jm):
                    x_grid[j, :] = x_grid[ref_ny, :]
                    y_grid[j, :] = y_grid[ref_ny, :]

            if ref_nx < im:
                for j in range(jm):
                    x_grid[j, ref_nx:] = x_grid[j, ref_nx - 1]
                    y_grid[j, ref_nx:] = y_grid[j, ref_nx - 1]

            return generate_grid_from_stereo(x_grid, y_grid, self.params)
